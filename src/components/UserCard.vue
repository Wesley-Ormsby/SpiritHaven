...
const { data } = defineProps<{ data: UserData | null }>()
</script>

<template>
  <RouterLink class="router-link" v-if="data" :to="{ name: 'profile', params: { id: data.id } }"
    ><Card class="user-card">
      <template #content>
        <div class="user-flex">
          <SpiritAvatar :spirit="data.spirit" class="user-avatar"></SpiritAvatar>
